...
def hc2mqtt(
    ...
    def on_connect(client, userdata, flags, rc):
        if rc == 5:
            ...
            for device in devices:
                ...
                mqtt_update_topic = f"{mqtt_prefix}{device['name']}/update"
                print(now(), device["name"], f"update topic: {mqtt_set_topic}")
    ...
    def on_message(client, userdata, msg):
        ...
        try:
            ...
            if topic == "update":
                msg = json.dumps(devState[device_name])
                print(now(), device_name, f"publish to {mqtt_prefix}{device_name} with {msg}")
                client.publish(f"{mqtt_prefix}{device_name}/state", msg)
                return
            elif topic == "set":
                resource = "/ro/values"
